...
class Controller extends BaseController
{
    ...
    public function server(Request $request)
    {
        ...
        switch ($action) {
            ...
            case 'listimage':
                ...
                if (config('UEditorUpload.core.mode') == 'local') {
                    ...
                    //     $request))->getList();
                    $oss_file_prefix = config('UEditorUpload.core.oss.oss_file_prefix');
                    $list = \OSS::list($oss_file_prefix);
                    $result = [
                        'state' => 'SUCCESS',
                        'list' => $list
                    ];
        ...
        if($callback = $request->get('callback', null)){
            return response($callback.'('.json_encode($result).')', 200);
        }
