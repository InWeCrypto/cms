...
class UploadFile  extends Upload{
    ...
    public function doUpload()
    {
        ...
        if(config('UEditorUpload.core.mode')=='local'){
            try {
                // 本地文件
                $local_file = $this->file->getPathName();
                // 线上文件
                $oss_file   = config('UEditorUpload.core.oss.oss_file_prefix', '/');
                $oss_file  .= md5_file($local_file) . '.' . $this->file->getClientOriginalExtension();

                $this->fullName = \OSS::updateFile($local_file, $oss_file);
